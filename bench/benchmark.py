...
async def one_request(client: httpx.AsyncClient, prompt: str) -> tuple[float, bool, bool]:
    start = time.perf_counter()
    try:
        r = await client.post(
            "/v1/chat/completions",
            json={"messages": [{"role": "user", "content": prompt}], "max_tokens": 64},
        )
...
async def run(base_url: str, n: int, concurrency: int, unique: bool) -> dict:
    ...
    async with httpx.AsyncClient(base_url=base_url, timeout=60) as client:
        async def worker(i: int):
            ...
            async with sem:
                latency, ok, cached = await one_request(client, prompt)
...
def main() -> None:
    ...
    report = asyncio.run(run(args.base_url, args.requests, args.concurrency, args.unique_prompts))
